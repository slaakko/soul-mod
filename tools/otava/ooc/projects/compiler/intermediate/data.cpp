...
{
    const BoolValue* value = static_cast<const BoolValue*>(this);
    return kind == ValueKind::boolValue && value->GetValue() == true;
}

bool Value::IsFalse() const noexcept
{
    const BoolValue* value = static_cast<const BoolValue*>(this);
    return kind == ValueKind::boolValue && value->GetValue() == false;
}
...
Value* Data::MakeIntegerLiteral(const soul::ast::Span& span, Type* type, const std::string& strValue, const Types& types)
{
    switch (type->Id())
    {
        ...
        case sbyteTypeId:
        {
            ...
            if (value < std::int8_min || value > std::int8_max)
            {
        ...
        {
            std::int64_t value = std::stoll(strValue);
            if (value < std::uint8_min || value > std::uint8_max)
            {
        ...
        {
            std::int64_t value = std::stoll(strValue);
            if (value < std::int16_min || value > std::int16_max)
            {
        ...
        {
            std::int64_t value = std::stoll(strValue);
            if (value < std::uint16_min || value > std::uint16_max)
            {
        ...
        {
            std::int64_t value = std::stoll(strValue);
            if (value < std::int32_min || value > std::int32_max)
            {
        ...
        {
            std::int64_t value = std::stoll(strValue);
            if (value < std::uint32_min || value > std::uint32_max)
            {
        ...
        {
            std::int64_t value = std::stoll(strValue);
            if (value < std::int64_min || value > std::int64_max)
            {
        ...
        case ulongTypeId:
        {
            ...
            if (value < std::uint64_min || value > std::uint64_max)
            {
