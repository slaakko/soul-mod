...
XmlProcessor::XmlProcessor(soul::xml::XmlContentHandler* contentHandler_) : lxr(nullptr), contentHandler(contentHandler_)
{
    if (!contentHandler)
    {
...
void XmlProcessor::EndStartTag()
{
    ...
    if (prefix == "xmlns")
    {
        throw XmlException("error: 'xmlns' prefix cannot be declared for an element in file " +
            lxr->FileName() + " line " + std::to_string(currentTag.sourcePos.line) + ":\n" + lxr->ErrorLines(currentTag.sourcePos.pos), currentTag.sourcePos);
    }
...
void XmlProcessor::EndTag(const soul::ast::SourcePos& sourcePos, const std::string& tagName)
{
    ...
    if (tagName != currentTag.name)
    {
        throw soul::xml::XmlException("error: end tag '" + tagName + "' does not match start tag '" + currentTag.name + "' in file " +
            lxr->FileName() + " line " + std::to_string(sourcePos.line) + ":\n" + lxr->ErrorLines(sourcePos.pos) +
            "\nsee reference line " + std::to_string(currentTag.sourcePos.line) + ":\n" + lxr->ErrorLines(currentTag.sourcePos.pos),
            sourcePos);
    }
    std::string localName;
    std::string prefix;
