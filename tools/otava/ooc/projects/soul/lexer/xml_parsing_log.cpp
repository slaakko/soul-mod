...
XmlParsingLog::XmlParsingLog(std::ostream& stream_) noexcept : ParsingLog(), formatter(stream_)
{
    formatter.SetIndentSize(1);
}

XmlParsingLog::XmlParsingLog(std::ostream& stream_, int maxLineLength_) noexcept : ParsingLog(maxLineLength_), formatter(stream_)
{
    formatter.SetIndentSize(1);
}

void XmlParsingLog::IncIndent() noexcept
{
    formatter.IncIndent();
}

void XmlParsingLog::DecIndent() noexcept
{
    formatter.DecIndent();
}
