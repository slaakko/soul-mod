...
std::expected<bool, int> MetadataRef::Write(util::CodeFormatter& formatter)
{
...
std::expected<bool, int> MetadataBool::Write(util::CodeFormatter& formatter)
{
    if (value)
    {
...
std::expected<bool, int> MetadataLong::Write(util::CodeFormatter& formatter)
{
...
std::expected<bool, int> MetadataString::Write(util::CodeFormatter& formatter)
{
    formatter.Write("\"");
    std::expected<std::string, int> rv = util::StringStr(value);
...
std::expected<bool, int> MetadataStruct::Write(util::CodeFormatter& formatter)
{
    formatter.Write("!" + std::to_string(id));
    return std::expected<bool, int>(true);
}

std::expected<bool, int> MetadataStruct::WriteDefinition(util::CodeFormatter& formatter)
{
    formatter.Write("!" + std::to_string(id) + " = {");
    bool first = true;
    for (const auto& item : itemMap)
    {
        ...
        formatter.Write(": ");
        auto rv = item.second->Write(formatter);
        if (!rv) return rv;
    }
    formatter.WriteLine("}");
    return std::expected<bool, int>(true);
...
{
    return AddMetadataStruct(soul::ast::Span(), static_cast<std::int32_t>(metadataNodes.size()));
}
...
std::expected<bool, int> Metadata::Write(util::CodeFormatter& formatter)
{
    ...
    {
        std::expected<bool, int> rv = node->WriteDefinition(formatter);
        if (!rv) return rv;
    }
    formatter.DecIndent();
    formatter.WriteLine("}");
