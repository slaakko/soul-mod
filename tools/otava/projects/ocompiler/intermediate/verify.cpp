...
void VerifierVisitor::CheckSameType(const std::string& typeDescription, Type* type, const std::string& expectedTypeDescription, Type* expected, const soul::ast::Span& span)
{
    ...
    if (type != expected)
    {
        auto rv = Error("type check error: " + typeDescription + " is '" + type->Name() + "' but " + expectedTypeDescription + " '" + expected->Name() + "' expected", span, GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::CheckType(const std::string& typeDescription, Type* type, Type* assertedType, const std::string& expectedTypeDescription, Type* expected, const soul::ast::Span& span)
{
    ...
    {
        auto rv = Error("type check error: " + typeDescription + " is '" + type->Name() + "', and " + expectedTypeDescription + " is '" + assertedType->Name() +
...
void VerifierVisitor::CheckBinaryInstructionTypes(BinaryInstruction& inst)
{
    ...
    {
        auto rv = Error("type check error: equal types expected: types are '" + inst.Result()->GetType()->Name() + "', '" + inst.Left()->GetType()->Name() + "' and '" +
...
void VerifierVisitor::CheckBooleanInstructionTypes(BinaryInstruction& inst)
{
    ...
    {
        auto rv = Error("type check error: operand types '" + inst.Left()->GetType()->Name() + "' and '" + inst.Right()->GetType()->Name() + "' differ", inst.Span(), GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::CheckArithmeticType(Type* type, const std::string& typeDescription, const soul::ast::Span& span)
{
    ...
    {
        auto rv = Error("type check error: arithmetic type expected, note: " + typeDescription + " is '" + type->Name() + "'", span, GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::CheckArithmeticOrBooleanType(Type* type, const std::string& typeDescription, const soul::ast::Span& span)
{
    ...
    {
        auto rv = Error("type check error: arithmetic or Boolean type expected, note: " + typeDescription + " is '" + type->Name() + "'", span, GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::CheckArithmeticOrPointerType(Type* type, const std::string& typeDescription, const soul::ast::Span& span)
{
    ...
    {
        auto rv = Error("type check error: arithmetic or pointer type expected, note: " + typeDescription + " is '" + type->Name() + "'", span, GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::CheckIntegerType(Type* type, const std::string& typeDescription, const soul::ast::Span& span)
{
    ...
    {
        auto rv = Error("type check error: integer type expected, note: " + typeDescription + " is '" + type->Name() + "'", span, GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::CheckIntegerOrBooleanType(Type* type, const std::string& typeDescription, const soul::ast::Span& span)
{
    ...
    {
        auto rv = Error("type check error: integer or Boolean type expected, note: " + typeDescription + " is '" + type->Name() + "'", span, GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::CheckFloatingPointType(Type* type, const std::string& typeDescription, const soul::ast::Span& span)
{
    ...
    {
        auto rv = Error("type check error: floating-point type expected, note: " + typeDescription + " is '" + type->Name() + "'", span, GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::CheckFloatType(Type* type, const std::string& typeDescription, const soul::ast::Span& span)
{
    ...
    if (!type->IsFloatType())
    {
        auto rv = Error("type check error: float type expected, note: " + typeDescription + " is '" + type->Name() + "'", span, GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::CheckDoubleType(Type* type, const std::string& typeDescription, const soul::ast::Span& span)
{
    ...
    {
        auto rv = Error("type check error: double type expected, note: " + typeDescription + " is '" + type->Name() + "'", span, GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::CheckPointerType(Type* type, const std::string& typeDescription, const soul::ast::Span& span)
{
    ...
    {
        auto rv = Error("type check error: pointer type expected, note: " + typeDescription + " is '" + type->Name() + "'", span, GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::CheckArithmeticPointerOrBooleanType(Type* type, const std::string& typeDescription, const soul::ast::Span& span)
{
    ...
    {
        auto rv = Error("type check exception: Boolean, arithmetic or pointer type expected, note: " + typeDescription + " is '" + type->Name() + "'", span, GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::CheckArithmeticPointerFunctionOrBooleanType(Type* type, const std::string& typeDescription, const soul::ast::Span& span)
{
    ...
    {
        auto rv = Error("type check exception: Boolean, arithmetic, pointer or function type expected, note: " + typeDescription + " is '" + type->Name() + "'", span, GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::CheckRegValue(Value* value, const soul::ast::Span& span)
{
    ...
    if (value->IsRegValue())
    {
        ...
        if (regValue->Reg() >= 0 && regValue->Reg() < regNumber)
        {
            ...
            {
                auto rv = Error("code verification error: instruction " + std::to_string(regValue->Reg()) + " not found from function '" + currentFunction->Name() + "'",
                    span, GetContext());
                SetError(rv.error());
                return;
            }
        ...
        {
            auto rv = Error("code verification error: invalid register value " + std::to_string(regValue->Reg()) +
...
void VerifierVisitor::CheckValueInstruction(ValueInstruction* valueInstruction)
{
    ...
    {
        auto rv = Error("code verification error: result register value " + std::to_string(regNumber) + " expected", valueInstruction->Span(), GetContext());
        SetError(rv.error());
        return;
    }
}
...
void VerifierVisitor::Visit(Function& function)
{
    ...
    {
        auto rv = Error("code verification error: function '" + function.Name() + "' has wrong number of parameters: " + std::to_string(numParams) +
...
void VerifierVisitor::Visit(BasicBlock& basicBlock)
{
    ...
    {
        auto rv = Error("code verification error: basic block " + std::to_string(basicBlock.Id()) + " is empty", basicBlock.Span(), GetContext());
        SetError(rv.error());
        return;
    }
    if (!basicBlock.LastInstruction()->IsTerminator())
    {
        auto rv = Error("code verification error: the last instruction '" + basicBlock.LastInstruction()->Name() + "' of basic block " + std::to_string(basicBlock.Id()) +
...
void VerifierVisitor::Visit(StoreInstruction& inst)
{
    ...
    if (!Valid()) return;
    std::expected<Type*, int> rv = inst.GetValue()->GetType()->AddPointer(GetContext());
    if (!rv)
    {
        SetError(rv.error());
        return;
    ...
    Type* ptrType = *rv;
    CheckType("type of value", inst.GetValue()->GetType(), ptrType, "pointer type", inst.GetPtr()->GetType(), inst.Span());
    if (!Valid()) return;
    inst.AddToUses();
    inst.SetIndex(index++);
}
...
void VerifierVisitor::Visit(JmpInstruction& inst)
{
    ...
    if (!targetBasicBlock)
    {
        auto rv = Error("code verification error: jump target basic block " + std::to_string(target) + " not found from function '" + function->Name() + "'", inst.Span(), GetContext());
        SetError(rv.error());
        return;
    }
    inst.SetTargetBasicBlock(targetBasicBlock);
    if (static_cast<Instruction*>(&inst) != basicBlock->LastInstruction())
    {
        auto rv = Error("code verification error: terminator in the middle of basic block " + std::to_string(basicBlock->Id()), inst.Span(), GetContext());
        SetError(rv.error());
        return;
    }
    inst.AddToUses();
...
void VerifierVisitor::Visit(BranchInstruction& inst)
{
    ...
    {
        auto rv = Error("code verification error: branch true target basic block " + std::to_string(trueTarget) + " not found from function '" + trueFunction->Name() + "'",
    ...
    {
        auto rv = Error("code verification error: branch false target basic block " + std::to_string(falseTarget) + " not found from function '" + falseFunction->Name() + "'",
            inst.Span(), GetContext());
        SetError(rv.error());
        return;
    }
    inst.SetFalseTargetBasicBlock(falseTargetBasicBlock);
    if (static_cast<Instruction*>(&inst) != inst.Parent()->LastInstruction())
    {
        auto rv = Error("code verification error: terminator in the middle of basic block " + std::to_string(inst.Parent()->Id()), inst.Span(), GetContext());
        SetError(rv.error());
        return;
    }
    inst.AddToUses();
...
void VerifierVisitor::Visit(ProcedureCallInstruction& inst)
{
    ...
    {
        std::expected<Type*, int> rv = calleeType->RemovePointer(inst.Span(), GetContext());
        if (!rv)
        {
            SetError(rv.error());
            return;
    ...
    if (calleeType->IsFunctionType())
    {
        ...
        {
            auto rv = Error("code verification error: function call has wrong number of arguments: " + std::to_string(functionType->Arity()) + " arguments expected: note: " +
        ...
        if (inst.Callee()->IsSymbolValue())
        {
            ...
            {
                auto rv = Error("internal error: could not add function declaration", inst.Span(), GetContext());
                SetError(rv.error());
                return;
            }
        }
    ...
    {
        auto rv = Error("type check error: callee in function call has invalid type: function type or function pointer type expected: note: type is " + inst.Callee()->GetType()->Name(),
            inst.Span(), GetContext());
        SetError(rv.error());
        return;
    }
...
void VerifierVisitor::Visit(SwitchInstruction& inst)
{
    ...
    {
        auto rv = Error("code verification error: switch default target basic block " + std::to_string(defaultTarget) +
    ...
    for (CaseTarget& caseTarget : inst.CaseTargets())
    {
        ...
        if (!caseTargetBlock)
        {
            auto rv = Error("code verification error: switch case target basic block " + std::to_string(caseTarget.targetLabelId) +
                " not found from function '" + function->Name() + "'", inst.Span(), GetContext());
            SetError(rv.error());
            return;
        }
...
void VerifierVisitor::Visit(SignExtendInstruction& inst)
{
    ...
    {
        auto rv = Error("code verification error: result type width expected to be greater than operand type width", inst.Span(), GetContext());
        SetError(rv.error());
        return;
    }
    CheckUnaryInstuction(&inst);
...
void VerifierVisitor::Visit(TruncateInstruction& inst)
{
    ...
    {
        auto rv = Error("code verification error: result type width expected to be less than operand type width", inst.Span(), GetContext());
        SetError(rv.error());
        return;
    }
    CheckUnaryInstuction(&inst);
...
void VerifierVisitor::Visit(BitcastInstruction& inst)
{
    ...
    if (inst.Result()->GetType()->Size() != inst.Operand()->GetType()->Size())
    {
        auto rv = Error("code verification error: result type width expected to be same as operand type width", inst.Span(), GetContext());
        SetError(rv.error());
        return;
    }
    CheckUnaryInstuction(&inst);
...
{
    if (!Valid()) return;
    std::expected<Type*, int> rv = inst.LocalType()->AddPointer(GetContext());
    if (!rv)
    {
        SetError(rv.error());
        return;
    ...
    Type* ptrType = *rv;
    CheckType("type of local", inst.LocalType(), ptrType, "instruction result type", inst.Result()->GetType(), inst.Span());
    if (!Valid()) return;
    CheckValueInstruction(&inst);
    if (!Valid()) return;
    inst.AddToUses();
...
void VerifierVisitor::Visit(LoadInstruction& inst)
{
    ...
    if (!Valid()) return;
    std::expected<Type*, int> rv = inst.Result()->GetType()->AddPointer(GetContext());
    if (!rv)
    {
        SetError(rv.error());
        return;
    ...
    Type* ptrType = *rv;
    CheckType("result type", inst.Result()->GetType(), inst.Ptr()->GetType(), "pointer to result type", ptrType, inst.Span());
    if (!Valid()) return;
...
void VerifierVisitor::Visit(ElemAddrInstruction& inst)
{
    ...
    if (baseType->IsStructureType())
    {
        ...
        if (inst.IndexValue()->IsIntegerValue())
        {
            ...
            {
                auto rv = Error("code verification error: invalid structure field index", inst.Span(), GetContext());
                SetError(rv.error());
                return;
            }
        }
    ...
    else if (baseType->IsArrayType())
    {
        ...
        if (inst.IndexValue()->IsIntegerValue())
        {
            ...
            {
                auto rv = Error("code verification error: invalid array element index", inst.Span(), GetContext());
                SetError(rv.error());
                return;
            }
        }
...
std::expected<bool, int> Verify(Context& context)
{
    VerifierVisitor visitor(&context);
    context.GetCode().VisitFunctions(visitor);
    if (!visitor) return std::unexpected<int>(visitor.GetError());
