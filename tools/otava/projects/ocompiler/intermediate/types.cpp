...
std::expected<Type*, int> Type::AddPointer(Context* context) const
{
    if (IsPointerType())
    {
        ...
        std::expected<PointerType*, int> rv = context->GetTypes().MakePointerType(
            Span(), GetBaseTypeId(pointerType->Id()), GetPointerCount(pointerType->Id()) + static_cast<std::int8_t>(1), context);
        if (!rv) return std::unexpected<int>(rv.error());
        return std::expected<Type*, int>(*rv);
    }
    else
    {
        std::expected<PointerType*, int> rv = context->GetTypes().MakePointerType(Span(), Id(), static_cast<std::int8_t>(1), context);
        if (!rv) return std::unexpected<int>(rv.error());
        return std::expected<Type*, int>(*rv);
    }
}

std::expected<Type*, int> Type::RemovePointer(const soul::ast::Span& span, Context* context) const
{
    if (IsPointerType())
    {
        const PointerType* pointerType = static_cast<const PointerType*>(this);
        return std::expected<Type*, int>(pointerType->BaseType());
    }
    else
    {
        return Error("pointer type expected", span, context);
    }
}
...
{
    std::expected<Type*, int> rv = RemovePointer(span, context);
    if (!rv) return std::unexpected<int>(rv.error());
    Type* pointeeType = *rv;
    if (pointeeType)
    {
        ...
        {
            return std::expected<StructureType*, int>(static_cast<StructureType*>(pointeeType));
        }
        else
        {
            return Error("structure type expected", span, context);
        }
    }
    else
    {
...
std::expected<ArrayType*, int> Type::GetArrayPointeeType(const soul::ast::Span& span, Context* context) const
{
    std::expected<Type*, int> rv = RemovePointer(span, context);
    if (!rv) return std::unexpected<int>(rv.error());
    Type* pointeeType = *rv;
    if (pointeeType)
    {
        ...
        {
            return std::expected<ArrayType*, int>(static_cast<ArrayType*>(pointeeType));
        }
        else
        {
            return Error("array type expected", span, context);
        }
    }
    else
    {
...
std::expected<bool, int> Type::Add(Types* types, Context* context)
{
    return std::expected<bool, int>(true);
}

std::expected<bool, int> Type::Resolve(Types* types, Context* context)
{
    return std::expected<bool, int>(true);
}
...
{
    return context.GetSByteValue(static_cast<std::int8_t>(0));
}
...
{
    return context.GetByteValue(static_cast<std::uint8_t>(0u));
}
...
{
    return context.GetShortValue(static_cast<std::int16_t>(0));
}
...
{
    return context.GetUShortValue(static_cast<std::uint16_t>(0u));
}
...
std::expected<bool, int> StructureType::Add(Types* types, Context* context)
{
    return types->Add(this, context);
}

std::expected<bool, int> StructureType::Resolve(Types* types, Context* context)
{
    for (auto& typeRef : fieldTypeRefs)
    {
        std::expected<bool, int> rv = types->ResolveType(typeRef, context);
        if (!rv) return rv;
    }
    return std::expected<bool, int>(true);
}
...
std::expected<bool, int> StructureType::ReplaceForwardReference(FwdDeclaredStructureType* fwdDeclaredType, StructureType* structureType, Context* context)
{
    int n = FieldCount();
    for (int i = 0; i < n; ++i)
    {
        ...
        if (fieldType == fwdDeclaredType)
        {
            ...
            std::expected<bool, int> rv = context->GetTypes().ResolveType(fieldTypeRefs[i], context);
            if (!rv) return rv;
        }
...
std::expected<bool, int> ArrayType::Add(Types* types, Context* context)
{
    return types->Add(this, context);
}

std::expected<bool, int> ArrayType::Resolve(Types* types, Context* context)
{
    return types->ResolveType(elementTypeRef, context);
}
...
std::expected<bool, int> FunctionType::Add(Types* types, Context* context)
{
    return types->Add(this, context);
}

std::expected<bool, int> FunctionType::Resolve(Types* types, Context* context)
{
    std::expected<bool, int> rv = types->ResolveType(returnTypeRef, context);
    if (!rv) return rv;
    for (auto& paramTypeRef : paramTypeRefs)
    {
        rv = types->ResolveType(paramTypeRef, context);
        if (!rv) return rv;
    }
    return std::expected<bool, int>(true);
}
...
std::expected<Type*, int> GetElemType(Value* ptr, Value* index, const soul::ast::Span& span, Context* context)
{
    ...
    if (type->IsPointerType())
    {
        ...
        if (aggregateType->IsStructureType())
        {
            ...
            {
                LongValue* longValue = static_cast<LongValue*>(index);
                std::int64_t idx = longValue->GetValue();
                if (idx >= 0 && idx < structureType->FieldCount())
                {
                    return std::expected<Type*, int>(context->MakePtrType(structureType->FieldType(static_cast<int>(idx))));
                }
                else
                {
                    return Error("invalid structure element type index", span, context);
                }
            }
            else
            {
                return Error("long valued index expected", span, context);
            }
        }
        else if (aggregateType->IsArrayType())
        {
            ArrayType* arrayType = static_cast<ArrayType*>(aggregateType);
            return std::expected<Type*, int>(context->MakePtrType(arrayType->ElementType()));
        }
        else if (aggregateType->IsFwdDeclaredStructureType())
        {
            ...
            return Error("forward declaration of structure type id " + std::to_string(aggregateType->Id()) + " (" + fwdType->Comment() + ") not resolved", span, context);
        }
        else
        {
            return Error("structure or array type expected", span, context);
        }
    }
    else
    {
...
void Types::Init()
{
    ...
    sbyteType.SetDefaultValue(context->GetSByteValue(static_cast<std::int8_t>(0)));
    byteType.SetDefaultValue(context->GetByteValue(static_cast<std::uint8_t>(0u)));
    shortType.SetDefaultValue(context->GetShortValue(static_cast<std::int16_t>(0)));
    ushortType.SetDefaultValue(context->GetUShortValue(static_cast<std::uint16_t>(0u)));
...
std::expected<bool, int> Types::Add(Type* type, Context* context)
{
    Type* prev = Get(type->Id());
    if (prev)
    {
        return Error("error adding type id " + std::to_string(type->Id()) + ": type id not unique", type->Span(), context, prev->Span());
    }
    Map(type);
...
std::expected<PointerType*, int> Types::MakePointerType(const soul::ast::Span& span, std::int32_t baseTypeId, std::int8_t pointerCount, Context* context)
{
    auto it = pointerTypeMap.find(std::make_pair(baseTypeId, pointerCount));
    if (it != pointerTypeMap.cend())
    {
        return std::expected<PointerType*, int>(it->second);
    }
    PointerType* type = nullptr;
    if (pointerCount > 1)
    {
        type = new PointerType(span, MakePointerTypeId(baseTypeId, pointerCount), pointerCount, MakePointerTypeId(baseTypeId, pointerCount - static_cast<std::int8_t>(1)));
    ...
    {
        return Error("pointer count > 0 expected", span, context);
    }
    std::expected<bool, int> rv = ResolveType(type->BaseTypeRef(), context);
    if (!rv) return std::unexpected<int>(rv.error());
    types.push_back(std::unique_ptr<Type>(type));
    Map(type);
    pointerTypeMap[std::make_pair(baseTypeId, pointerCount)] = type;
    return std::expected<PointerType*, int>(type);
}

std::expected<Type*, int> Types::MakePtrType(Type* baseType)
{
    std::expected<Type*, int> rv = baseType->AddPointer(context);
    if (!rv) return rv;
    Type* type = *rv;
...
std::expected<StructureType*, int> Types::GetStructureType(const soul::ast::Span& span, std::int32_t typeId, const std::vector<TypeRef>& fieldTypeRefs)
{
    ...
    if (it != structureTypeMap.cend())
    {
        ...
        return std::expected<StructureType*, int>(structureType);
    }
    StructureType* structureType = new StructureType(span, typeId, fieldTypeRefs);
    types.push_back(std::unique_ptr<Type>(structureType));
    std::expected<bool, int> rv = structureType->Add(this, context);
    if (!rv) return std::unexpected<int>(rv.error());
...
std::expected<ArrayType*, int> Types::GetArrayType(const soul::ast::Span& span, std::int32_t typeId, std::int64_t size, const TypeRef& elementTypeRef)
{
    ...
    if (it != arrayTypeMap.cend())
    {
        ...
        return std::expected<ArrayType*, int>(arrayType);
    }
    ...
    std::expected<bool, int> rv = arrayType->Add(this, context);
    if (!rv) return std::unexpected<int>(rv.error());
    return std::expected<ArrayType*, int>(arrayType);
}

std::expected<FunctionType*, int> Types::GetFunctionType(const soul::ast::Span& span, std::int32_t typeId, const TypeRef& returnTypeRef, const std::vector<TypeRef>& paramTypeRefs)
{
    ...
    if (it != functionTypeMap.cend())
    {
        ...
        return std::expected<FunctionType*, int>(functionType);
    }
    ...
    std::expected<bool, int> rv = functionType->Add(this, context);
    if (!rv) return std::unexpected<int>(rv.error());
    return std::expected<FunctionType*, int>(functionType);
}
...
std::expected<bool, int> Types::Resolve(Context* context)
{
    for (const auto& type : types)
    {
        std::expected<bool, int> rv = type->Add(this, context);
        if (!rv) return rv;
    }
    ...
    {
        std::expected<bool, int> rv = type->Resolve(this, context);
        if (!rv) return rv;
    }
    return std::expected<bool, int>(true);
}

std::expected<bool, int> Types::ResolveType(TypeRef& typeRef, Context* context)
{
    ...
    if (!type)
    {
        ...
        {
            std::expected<PointerType*, int> rv = MakePointerType(typeRef.Span(), GetBaseTypeId(typeRef.Id()), GetPointerCount(typeRef.Id()), context);
            if (!rv) return std::unexpected<int>(rv.error());
            type = *rv;
        }
    }
    if (!type)
    {
        return Error("error resolving type: type id " + std::to_string(typeRef.Id()) + " not found", typeRef.Span(), context);
    }
