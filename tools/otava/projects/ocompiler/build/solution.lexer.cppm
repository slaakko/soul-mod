...
template<typename CharT>
std::expected<soul::lexer::Lexer<SolutionLexer<CharT>, CharT>, int> MakeLexer(const CharT* start, const CharT* end, const std::string& fileName);

template<typename CharT>
std::expected<soul::lexer::Lexer<SolutionLexer<CharT>, CharT>, int> MakeLexer(const std::string& moduleFileName, util::ResourceFlags resourceFlags, const CharT* start, const CharT* end, const std::string& fileName);
    ...
    static std::int64_t GetTokenId(std::int32_t ruleIndex, soul::lexer::LexerBase<CharT>& lexer)
    {
        switch (ruleIndex)
        {
            ...
            case 1:
            {
                ...
                if (kw == soul::lexer::INVALID_TOKEN) return otava::solution_token::ID;
                else return kw;
                break;
            }
            case 2:
            {
                lexer.Retract();
                return otava::solution_token::FILEPATH;
                break;
            }
            case 3:
            {
                lexer.Retract();
                return otava::solution_token::SEMICOLON;
                break;
            }
            case 4:
            {
                lexer.Retract();
                return otava::solution_token::DOT;
                break;
            }
        }
...
std::expected<soul::lexer::ClassMap<CharT>*, int> GetClassMap()
{
    ...
    if (!classmap)
    {
        std::expected<soul::lexer::ClassMap<CharT>*, int> rv = soul::lexer::MakeClassMap<CharT>("otava.solution_lexer.classmap");
...
std::expected<soul::lexer::ClassMap<CharT>*, int> GetClassMap(const std::string & moduleFileName, util::ResourceFlags resourceFlags)
{
    ...
    if (!classmap)
    {
        std::expected<soul::lexer::ClassMap<CharT>*, int> rv = soul::lexer::MakeClassMap<CharT>(moduleFileName, "otava.solution_lexer.classmap", resourceFlags);
        if (!rv) return std::unexpected<int>(rv.error());
        classmap = *rv;
    }
    return classmap;
...
std::expected<soul::lexer::Lexer<SolutionLexer<CharT>, CharT>, int> MakeLexer(const CharT* start, const CharT* end, const std::string& fileName)
{
    ...
    auto lexer = soul::lexer::Lexer<SolutionLexer<CharT>, CharT>(start, end, fileName);
...
template class SolutionLexer<char32_t>;
template class soul::lexer::Lexer<SolutionLexer<char32_t>, char32_t>;
