...
{
    std::expected<Value*, int> rv = Evaluate(condition, context);
    if (!rv) return false;
    Value* value = *rv;
...
void FunctionReturnPathChecker::Visit(otava::ast::CompoundStatementNode& node)
{
    ...
    std::unexpected<int> result = Error("not all control paths terminate in return statement", node.GetSourcePos(), context);
...
std::expected<bool, int> CheckFunctionReturnPaths(otava::ast::Node* node, Context* context)
{
    ...
    node->Accept(checker);
    if (!checker) return std::unexpected<int>(checker.Error());
