...
public:
    ...
    void Init();
    std::expected<std::u32string, int> GetGroupName(otava::ast::NodeKind nodeKind, const soul::ast::SourcePos& sourcePos, Context* context);
private:
...
void FirstArgResolver::Visit(BoundVariableNode& node)
{
    ...
    if (node.GetType()->IsReferenceType())
    {
        ...
        {
            std::expected<TypeSymbol*, int> t = node.GetType()->RemoveReference(context);
            if (!t)
            {
                SetError(t.error());
                return;
            ...
            type = *t;
            firstArg = new BoundRefToPtrNode(node.Clone(), node.GetSourcePos(), type);
        }
        else
        {
            std::expected<TypeSymbol*, int> t = node.GetType()->RemoveReference(context);
    ...
    else if (op == otava::ast::NodeKind::nullNode)
    {
        if (node.GetType()->IsClassTypeSymbol())
        {
            if (hasConstDerivation)
            {
                std::expected<TypeSymbol*, int> t = node.GetType()->AddPointer(context);
                if (!t)
                {
                    SetError(t.error());
                    return;
                ...
                type = *t;
                firstArg = new BoundAddressOfNode(node.Clone(), node.GetSourcePos(), type);
            }
            else
            {
                std::expected<TypeSymbol*, int> t = node.GetType()->AddPointer(context);
...
std::expected<BoundExpressionNode*, int> GetFirstArg(BoundNode* node, Context* context)
{
    FirstArgResolver resolver(context);
    node->Accept(resolver);
    if (!resolver) return std::unexpected<int>(resolver.GetError());
    return std::expected<BoundExpressionNode*, int>(resolver.GetFirstArg());
}
...
void ExpressionBinder::BindBinaryOp(otava::ast::NodeKind op, const soul::ast::SourcePos& sourcePos, BoundExpressionNode* left, BoundExpressionNode* right)
{
    ...
    std::expected<TypeSymbol*, int> pt = left->GetType()->AddPointer(context);
    if (!pt)
    {
        SetError(pt.error());
        return;
    ...
    if (functionSymbol->ReturnsClass())
    {
        ...
        classTemporary = *rv;
        std::expected<TypeSymbol*, int> pt = classTemporary->GetType()->AddPointer(context);
        if (!pt)
        {
            SetError(pt.error());
            return;
        }
        functionCall->AddArgument(new BoundAddressOfNode(new BoundVariableNode(classTemporary, sourcePos), sourcePos, *pt));
    }
...
void ExpressionBinder::BindUnaryOp(otava::ast::NodeKind op, const soul::ast::SourcePos& sourcePos, BoundExpressionNode* operand)
{
    ...
    if (!functionCall)
    {
        ...
        std::expected<TypeSymbol*, int> pt = args[0]->GetType()->AddPointer(context);
        if (!pt)
        {
            SetError(pt.error());
            return;
...
void ExpressionBinder::BindClassArrow(BoundExpressionNode* operand, otava::ast::MemberExprNode* memberExprNode)
{
    ...
    {
        std::expected<TypeSymbol*, int> pt = operand->GetType()->RemoveReference(context);
        if (!pt)
        {
            SetError(pt.error());
            return;
        ...
        type = *pt;
        operand = new BoundRefToPtrNode(operand, memberExprNode->GetSourcePos(), type);
    }
    else
    {
        std::expected<TypeSymbol*, int> pt = operand->GetType()->AddPointer(context);
        if (!pt)
        {
            SetError(pt.error());
            return;
        ...
        TypeSymbol* type = *pt;
        operand = new BoundAddressOfNode(operand, memberExprNode->GetSourcePos(), type);
    }
...
void ExpressionBinder::BindDeref(const soul::ast::SourcePos& sourcePos, BoundExpressionNode* operand)
{
    ...
    {
        std::expected<TypeSymbol*, int> pt = operand->GetType()->RemovePointer(context);
        if (!pt)
        {
            SetError(pt.error());
            return;
        ...
        if (type->IsClassTypeSymbol())
        {
            ...
            type = *pt;
            boundExpression = new BoundPtrToRefNode(operand, sourcePos, type);
        }
        ...
        else
        {
            ...
            {
                boundExpression = new BoundDereferenceNode(operand, sourcePos, type, OperationFlags::derefAfterConv);
            }
            else
            {
                boundExpression = new BoundDereferenceNode(operand, sourcePos, type);
            }
        }
    }
    else
    {
        std::expected<TypeSymbol*, int> pt = operand->GetType()->PlainType(context);
        if (!pt)
        {
            SetError(pt.error());
            return;
        }
        TypeSymbol* plainSubjectType = *pt;
        if (plainSubjectType->IsClassTypeSymbol())
        {
            if (operand->GetType()->IsReferenceType())
            {
                ...
                type = *pt;
                boundExpression = new BoundRefToPtrNode(operand, sourcePos, type);
            }
            else
            {
                std::expected<TypeSymbol*, int> pt = operand->GetType()->AddPointer(context);
                ...
                TypeSymbol* type = *pt;
                boundExpression = new BoundAddressOfNode(operand, sourcePos, type);
            }
...
void ExpressionBinder::BindPrefixInc(const soul::ast::SourcePos& sourcePos, BoundExpressionNode* operand, otava::ast::Node* child)
{
    ...
    {
        std::expected<otava::ast::Node*, int> c1 = child->Clone();
        if (!c1)
        {
            SetError(c1.error());
            return;
        }
        otava::ast::Node* child1 = *c1;
        std::expected<otava::ast::Node*, int> c2 = child->Clone();
        if (!c2)
        {
            SetError(c2.error());
            return;
        }
        otava::ast::Node* child2 = *c2;
        otava::ast::BinaryExprNode assignmentExpr(sourcePos,
            new otava::ast::AssignNode(sourcePos),
            child1, new otava::ast::BinaryExprNode(sourcePos, new otava::ast::PlusNode(sourcePos), child2,
...
void ExpressionBinder::BindPrefixDec(const soul::ast::SourcePos& sourcePos, BoundExpressionNode* operand, otava::ast::Node* child)
{
    ...
    else
    {
        ...
        otava::ast::BinaryExprNode assignmentExpr(sourcePos,
            new otava::ast::AssignNode(sourcePos),
            child1,
            new otava::ast::BinaryExprNode(sourcePos, new otava::ast::MinusNode(sourcePos), child2,
                new otava::ast::IntegerLiteralNode(sourcePos, 1, otava::ast::Suffix::none, otava::ast::Base::decimal, U"")));
        context->PushSetFlag(ContextFlags::suppress_warning);
        std::expected<BoundExpressionNode*, int> rv = BindExpression(&assignmentExpr, context);
        if (!rv)
...
void ExpressionBinder::Visit(otava::ast::CppCastExprNode& node)
{
    ...
    std::expected<TypeSymbol*, int> dt = resultType->DirectType(context);
    if (!dt)
    {
        SetError(dt.error());
        return;
    ...
    rv = directType->FinalType(node.GetSourcePos(), context);
    if (!rv)
    {
        SetError(rv.error());
        return;
    ...
    dt = boundExpression->GetType()->DirectType(context);
    if (!dt)
    {
        SetError(dt.error());
        return;
    ...
    directType = *dt;
    std::expected<TypeSymbol*, int> ftv = directType->FinalType(node.GetSourcePos(), context);
    if (!ftv)
    {
        SetError(ftv.error());
        return;
    ...
    if (conversion)
    {
        ...
        {
            std::expected<BoundExpressionNode*, int> brv = MakeLvalueExpression(boundExpression, node.GetSourcePos(), context);
            if (!brv)
            {
                SetError(brv.error());
                return;
            }
            std::expected<TypeSymbol*, int> pt = boundExpression->GetType()->AddPointer(context);
            if (!pt)
            {
                SetError(pt.error());
                return;
            ...
            TypeSymbol* type = *pt;
            boundExpression = new BoundAddressOfNode(*brv, node.GetSourcePos(), type);
        }
        else if (argumentMatch.preConversionFlags == OperationFlags::deref)
        {
            std::expected<TypeSymbol*, int> pt = boundExpression->GetType()->RemoveReference(context);
            if (!pt)
            {
                SetError(pt.error());
                return;
            ...
            TypeSymbol* type = *pt;
            boundExpression = new BoundDereferenceNode(boundExpression, node.GetSourcePos(), type);
        }
...
void NewInitializerMaker::Visit(otava::ast::InvokeExprNode& node)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        std::expected<otava::ast::Node*, int> a = node.Items()[i]->Clone();
        if (!a)
        {
            SetError(a.error());
            return;
        }
        otava::ast::Node* arg = *a;
...
void ExpressionBinder::Visit(otava::ast::IdentifierNode& node)
{
    ...
    if (qualifiedScope)
    {
        std::expected<Symbol*, int> lrv = scope->Lookup(node.Str(), groups, ScopeLookup::thisAndBaseScopes, node.GetSourcePos(), context, LookupFlags::dontResolveSingle);
        if (!lrv)
        {
            SetError(lrv.error());
            return;
    ...
    if (!symbol)
    {
        std::expected<Symbol*, int> lrv = scope->Lookup(node.Str(), groups, ScopeLookup::allScopes, node.GetSourcePos(), context, LookupFlags::dontResolveSingle);
        if (!lrv)
        {
            SetError(lrv.error());
            return;
    ...
    if (!symbol)
    {
        ...
        if (sp)
        {
            std::expected<Symbol*, int> lrv = sp->ClassTemplate()->GetScope()->Lookup(node.Str(), groups, ScopeLookup::allScopes, node.GetSourcePos(), 
    ...
    if (!symbol && boundExpression && boundExpression->IsBoundTypeNode())
    {
        ...
        std::expected<Symbol*, int> lrv = type->GetScope()->Lookup(node.Str(), groups, ScopeLookup::allScopes, node.GetSourcePos(), 
            context, LookupFlags::dontResolveSingle);
        if (!lrv)
        {
            SetError(lrv.error());
    ...
    if (!symbol && (symbolGroups & SymbolGroupKind::functionSymbolGroup) != SymbolGroupKind::none)
    {
        ...
        if (qualifiedScope)
        {
            std::expected<Symbol*, int> lrv = scope->Lookup(node.Str(), groups, ScopeLookup::thisAndBaseScopes, node.GetSourcePos(), 
                context, LookupFlags::dontResolveSingle);
            if (!lrv)
            {
                SetError(lrv.error());
            ...
            case SymbolKind::variableGroupSymbol:
            {
                ...
                if (sym && sym->IsVariableSymbol())
                {
                    ...
                    if (variable->IsMemberVariable())
                    {
                        ...
                        boundVariable->SetThisPtr(thisPtr);
                    }
...
void ExpressionBinder::Visit(otava::ast::DestructorIdNode& node)
{
    ...
    std::expected<Symbol*, int> lrv = scope->Lookup(U"@destructor", SymbolGroupKind::functionSymbolGroup, ScopeLookup::thisScope, node.GetSourcePos(),
        context, LookupFlags::dontResolveSingle);
    if (!lrv)
    {
        SetError(lrv.error());
...
void ExpressionBinder::Visit(otava::ast::TemplateIdNode& node)
{
    ...
    std::expected<bool, int> prv = currentScope->PushParentScope(scope);
    if (!prv)
    {
        SetError(prv.error());
        return;
    ...
    prv = currentScope->PopParentScope();
    if (!prv)
    {
        SetError(prv.error());
        return;
...
{
    if (!Valid()) return;
    std::expected<TypeSymbol*, int> pt = subject->GetType()->PlainType(context);
    if (!pt)
    {
        SetError(pt.error());
        return;
    ...
    TypeSymbol* plainType = *pt;
    if (node->Op()->Kind() == otava::ast::NodeKind::arrowNode && plainType->IsClassTypeSymbol())
    {
    ...
    if (subject->IsBoundLocalVariable() && member->IsBoundMemberVariable())
    {
        ...
        else
        {
            ...
            {
                std::expected<TypeSymbol*, int> pt = localVar->GetType()->RemoveReference(context);
                if (!pt)
                {
                    SetError(pt.error());
                    return;
                ...
                type = *pt;
                thisPtr = new BoundRefToPtrNode(localVar, node->GetSourcePos(), type);
            }
            else
            {
                std::expected<TypeSymbol*, int> pt = localVar->GetType()->AddPointer(context);
                if (!pt)
                {
                    SetError(pt.error());
                    return;
                ...
                TypeSymbol* type = *pt;
                thisPtr = new BoundAddressOfNode(localVar, node->GetSourcePos(), type);
            }
    ...
    else if (subject->IsBoundMemberVariable() && member->IsBoundMemberVariable())
    {
        ...
        else
        {
            ...
            {
                std::expected<TypeSymbol*, int> pt = subjectVar->GetType()->RemoveReference(context);
                if (!pt)
                {
                    SetError(pt.error());
                    return;
                ...
                type = *pt;
                thisPtr = new BoundRefToPtrNode(subjectVar, node->GetSourcePos(), type);
            }
            else
            {
                std::expected<TypeSymbol*, int> pt = subjectVar->GetType()->AddPointer(context);
                if (!pt)
                {
                    SetError(pt.error());
                    return;
                ...
                TypeSymbol* type = *pt;
                thisPtr = new BoundAddressOfNode(subjectVar, node->GetSourcePos(), type);
            }
            memberVar->SetThisPtr(thisPtr);
        }
        boundExpression = memberVar;
    ...
    else if (subject->IsBoundParameterNode() && member->IsBoundMemberVariable())
    {
        ...
        if (param->GetType()->GetBaseType()->IsClassTypeSymbol())
        {
            ...
            else
            {
                ...
                {
                    std::expected<TypeSymbol*, int> pt = param->GetType()->RemoveReference(context);
                    if (!pt)
                    {
                        SetError(pt.error());
                        return;
                    ...
                    type = *pt;
                    thisPtr = new BoundRefToPtrNode(param, node->GetSourcePos(), type);
                }
                else
                {
                    std::expected<TypeSymbol*, int> pt = param->GetType()->AddPointer(context);
                    if (!pt)
                    {
                        SetError(pt.error());
                        return;
                    ...
                    TypeSymbol* type = *pt;
                    thisPtr = new BoundAddressOfNode(param, node->GetSourcePos(), type);
                }
            }
            BoundVariableNode* memberVar = static_cast<BoundVariableNode*>(member.release());
            memberVar->SetThisPtr(thisPtr);
...
void ExpressionBinder::Visit(otava::ast::InvokeExprNode& node)
{
    ...
    if (subject)
    {
        ...
        if (subject->IsBoundTypeNode())
        {
            ...
            temporary = *rv;
            std::expected<TypeSymbol*, int> pt = temporary->GetType()->AddPointer(context);
            if (!pt)
            {
                SetError(pt.error());
                return;
            }
            BoundAddressOfNode* boundAddrOfNode = new BoundAddressOfNode(new BoundVariableNode(temporary, node.GetSourcePos()), node.GetSourcePos(), *pt);
        ...
        {
            std::expected<BoundExpressionNode*, int> f = GetFirstArg(subject.get(), context);
            if (!f)
            {
                SetError(f.error());
                return;
            }
            BoundExpressionNode* firstArg = *f;
            if (firstArg)
            {
        ...
        if (functionSymbol->ReturnsClass())
        {
            ...
            functionCall->AddArgument(new BoundAddressOfNode(new BoundVariableNode(classTemporary, node.GetSourcePos()), node.GetSourcePos(), *pt));
        }
        boundExpression = functionCall.release();
        if (classTemporary)
        {
...
void ExpressionBinder::Visit(otava::ast::SubscriptExprNode& node)
{
    ...
    else if (plainSubjectType->IsArrayType())
    {
        ...
        TypeSymbol* arrayPtrType = *pt;
        pt = arrayType->ElementType()->AddPointer(context);
        TypeSymbol* elementPtrType = *pt;
        ...
        std::expected<FunctionSymbol*, int> rv = context->GetBoundCompileUnit()->GetArgumentConversionTable()->GetArgumentConversion(
            elementPtrType, arrayPtrType, node.GetSourcePos(), context);
        if (!rv)
        {
            SetError(rv.error());
            return;
...
void ExpressionBinder::Visit(otava::ast::NewExprNode& node)
{
    ...
    type = *trv;
    std::expected<otava::ast::Node*, int> c = node.Child()->Clone();
    if (!c)
    {
        SetError(c.error());
        return;
    }
    otava::ast::SizeOfTypeExprNode sizeOfNode(node.GetSourcePos(), *c, soul::ast::SourcePos(), soul::ast::SourcePos());
    ...
    std::expected<TypeSymbol*, int> pt = voidType->AddPointer(context);
    if (!pt)
    {
        SetError(pt.error());
        return;
    ...
    std::expected<VariableSymbol*, int> ttv = context->GetBoundFunction()->GetFunctionDefinitionSymbol()->CreateTemporary(ptrType);
    if (!ttv)
    {
        SetError(ttv.error());
        return;
    ...
    pt = tempVar->GetType()->AddPointer(context);
    if (!pt)
    {
        SetError(pt.error());
        return;
    }
    TypeSymbol* tempVarPtrType = *pt;
    ...
    std::expected<FunctionSymbol*, int> arv = context->GetBoundCompileUnit()->GetArgumentConversionTable()->GetArgumentConversion(ptrType, voidPtrType, 
        node.GetSourcePos(), context);
    if (!arv)
    {
        SetError(arv.error());
...
void ExpressionBinder::Visit(otava::ast::PostfixIncExprNode& node)
{
    ...
    if (!inhibitCompile)
    {
        std::expected<TypeSymbol*, int> pt = boundExpression->GetType()->PlainType(context);
        if (!pt)
        {
            SetError(pt.error());
            return;
        ...
        if (plainType->IsClassTypeSymbol())
        {
            ...
            if (boundExpression->GetType()->IsReferenceType())
            {
                ...
                type = *pt;
                args.push_back(std::unique_ptr<BoundExpressionNode>(new BoundRefToPtrNode(boundExpression, node.GetSourcePos(), type)));
            }
            else
            {
                std::expected<TypeSymbol*, int> pt = boundExpression->GetType()->AddPointer(context);
                ...
                TypeSymbol* type = *pt;
                args.push_back(std::unique_ptr<BoundExpressionNode>(new BoundAddressOfNode(boundExpression, node.GetSourcePos(), type)));
            }
        ...
        else
        {
            ...
            std::expected<otava::ast::Node*, int> c1 = node.Child()->Clone();
            if (!c1)
            {
                SetError(c1.error());
                return;
            }
            std::expected<otava::ast::Node*, int> c2 = node.Child()->Clone();
            if (!c2)
            {
                SetError(c2.error());
                return;
            }
            std::expected<otava::ast::Node*, int> c3 = node.Child()->Clone();
            if (!c3)
            {
                SetError(c3.error());
                return;
            }
            std::expected<VariableSymbol*, int> v = context->GetSymbolTable()->AddVariable(*trv, &node, type, type, nullptr, DeclarationFlags::none, context);
            if (!v)
            {
                SetError(v.error());
                return;
            }
            VariableSymbol* temporary = *v;
            ...
            otava::ast::BinaryExprNode expr(node.GetSourcePos(),
                ...
                new otava::ast::BinaryExprNode(node.GetSourcePos(),
                    ...
                    new otava::ast::BinaryExprNode(node.GetSourcePos(),
                        new otava::ast::AssignNode(node.GetSourcePos()),
                        *c2,
                        new otava::ast::BinaryExprNode(node.GetSourcePos(), new otava::ast::PlusNode(node.GetSourcePos()), *c3,
...
void ExpressionBinder::Visit(otava::ast::PostfixDecExprNode& node)
{
    ...
    if (!inhibitCompile)
    {
        ...
        if (plainType->IsClassTypeSymbol())
        {
            ...
            if (boundExpression->GetType()->IsReferenceType())
            {
                pt = boundExpression->GetType()->RemoveReference(context);
                if (!pt)
                {
                    SetError(pt.error());
                    return;
            ...
            {
                pt = boundExpression->GetType()->AddPointer(context);
                if (!pt)
                {
                    SetError(pt.error());
                    return;
            ...
            otava::ast::BinaryExprNode expr(node.GetSourcePos(),
                ...
                new otava::ast::BinaryExprNode(node.GetSourcePos(),
                    ...
                    new otava::ast::BinaryExprNode(node.GetSourcePos(),
                        new otava::ast::AssignNode(node.GetSourcePos()),
                        *c2,
                        new otava::ast::BinaryExprNode(node.GetSourcePos(), new otava::ast::MinusNode(node.GetSourcePos()), *c3,
                            new otava::ast::IntegerLiteralNode(node.GetSourcePos(), 1, otava::ast::Suffix::none, otava::ast::Base::decimal, U"")))),
                new otava::ast::IdentifierNode(node.GetSourcePos(), temporaryName));
            std::expected<BoundExpressionNode*, int> rv = BindExpression(&expr, context);
            if (!rv)
