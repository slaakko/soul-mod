...
std::expected<std::u32string, int> CharStr(char32_t c)
{
    ...
    default:
    {
        if (c >= static_cast<char32_t>(32) && c <= static_cast<char32_t>(126))
        {
...
std::string ToUpperNarrow(const std::string& s)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        result.append(1, static_cast<char>(std::toupper(s[i])));
    }
    return result;
}
...
std::string ToLowerNarrow(const std::string& s)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        result.append(1, static_cast<char>(std::tolower(s[i])));
    }
    return result;
}
...
{
    std::string s;
    s.append(1, HexNibble(static_cast<char>(x >> 4))).append(1, HexNibble(static_cast<char>(x & 0x0F)));
    return s;
}
...
{
    std::string s;
    s.append(ToHexString(std::uint8_t((x >> 24U) & 0xFFU)));
    s.append(ToHexString(std::uint8_t((x >> 16U) & 0xFFU)));
    s.append(ToHexString(std::uint8_t((x >> 8U) & 0xFFU)));
    s.append(ToHexString(std::uint8_t((x & 0xFFU))));
    return s;
}
...
std::string ToOctalString(std::int32_t value, int numDigits)
{
    ...
    {
        int digit = value & 7;
        str.append(1, static_cast<char>('0' + digit));
        value = value >> 3;
    }
