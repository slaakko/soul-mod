...
std::string DurationStr(int64_t nanosecs)
{
    std::string s;
...
std::time_t MkTime(const DateTime& dt)
{
    ...
    tm.tm_mon = static_cast<int>(static_cast<std::int8_t>(dt.GetDate().GetMonth())) - static_cast<int>(static_cast<std::int8_t>(Month::january));
...
std::string TimeToString(std::time_t time)
{
    ...
    char* p = &buffer[0];
    std::strftime(p, 20, "%d.%m.%Y %H:%M:%S", ptm);
    return std::string(p);
}
