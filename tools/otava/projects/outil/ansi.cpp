...
{
public:
    static AnsiEngine& Out();
    static AnsiEngine& Error();
    std::expected<std::string, int> Process(const std::string& text);
    void SetHandle(void* handle_) { handle = handle_; }
private:
    AnsiEngine();
    std::expected<bool, int> Put(char c);
    void Emit(char c);
    std::expected<bool, int> SetAttributes();
    std::expected<bool, int> ResetAttributes();
    int state;
    std::string outChars;
    bool charsReady;
...
static AnsiEngine& AnsiEngine::Out()
{
    static AnsiEngine out;
    out.SetHandle(ort_get_std_output_handle());
    return out;
}

static AnsiEngine& AnsiEngine::Error()
{
    static AnsiEngine error;
    error.SetHandle(ort_get_std_error_handle());
    return error;
}
...
std::expected<std::string, int> AnsiIntercept(int handle, const std::string& text)
{
    ...
    {
        return std::expected<std::string, int>(text);
    }
}
...
std::expected<std::string, int> AnsiEngine::Process(const std::string& text)
{
    std::string output;
    for (char c : text)
    {
        std::expected<bool, int> rv = Put(c);
    ...
    return std::expected<std::string, int>(output);
}

std::expected<bool, int> AnsiEngine::SetAttributes()
{
    if (attributesReset)
    {
        attributesReset = false;
        prevAttrs = ort_get_console_attrs(handle);
        if (prevAttrs == static_cast<std::uint16_t>(-1))
        {
            return std::expected<bool, int>(std::unexpected<int>(AllocateError("AnsiEngine: failed to fetch console attributes")));
        }
    }
    std::uint16_t attrs = prevAttrs;
    if (attributes.foreback == ForeBack::foreground)
    {
        attrs = (attrs & static_cast<uint16_t>(0xFF00u)) | static_cast<uint16_t>(static_cast<std::uint8_t>(ToConsoleColor(attributes.color, attributes.intensity)));
    }
    else if (attributes.foreback == ForeBack::background)
    {
        attrs = (attrs & static_cast<uint16_t>(0x00FFu)) | static_cast<uint16_t>(static_cast<std::uint8_t>(ToConsoleColor(attributes.color, attributes.intensity)) << 8);
    }
    if (!ort_set_console_text_attribute(handle, attrs))
    {
        return std::expected<bool, int>(std::unexpected<int>(AllocateError("AnsiEngine: failed to set console attributes")));
    }
    return std::expected<bool, int>(true);
}

std::expected<bool, int> AnsiEngine::ResetAttributes()
{
    attributesReset = true;
    if (!ort_set_console_text_attribute(handle, prevAttrs))
    {
        return std::expected<bool, int>(std::unexpected<int>(AllocateError("AnsiEngine: failed to reset console attributes")));
    }
    return std::expected<bool, int>(true);
}
...
std::expected<bool, int> AnsiEngine::Put(char c)
{
    switch (state)
    {
    case 0:
    ...
    case 999:
    {
        if (c == 'm')
        {
            std::expected<bool, int> rv = ResetAttributes();
            if (!rv) return rv;
        }
        else
        {
    ...
    case 1000:
    {
        if (c == 'm')
        {
            std::expected<bool, int> rv = SetAttributes();
            if (!rv) return rv;
        }
        else
        {
