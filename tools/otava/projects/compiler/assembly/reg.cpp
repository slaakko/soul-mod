...
RegisterGroup::RegisterGroup() : kind(RegisterGroupKind::max), nonvolatile(false), regs(16, Register())
{
}

RegisterGroup::RegisterGroup(RegisterGroupKind kind_) : kind(kind_), nonvolatile(false), regs(16, Register())
{
}
...
const Register* RegisterGroup::GetReg(std::int64_t size) const
{
    ...
    {
        const Register* reg = &regs[size - 1];
        return reg;
    }
    else
    {
