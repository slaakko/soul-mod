...
bool Project::UpToDate(const std::string& config, int optLevel, int error) const
{
    ...
    {
        std::filesystem::path p = outputFilePath;
        std::filesystem::path r = reference->OutputFilePath();
        if (!std::filesystem::exists(r) ||
            std::filesystem::last_write_time(p) < std::filesystem::last_write_time(r))
        {
            return false;
        }
    }
    for (const auto& module : modules)
    {
        if (module)
        {
            ...
            if (module->Kind() == otava::symbols::ModuleKind::interfaceModule)
            {
                ...
                std::filesystem::path mp = moduleFilePath;
                std::filesystem::path sp = moduleSourceFilePath;
                if (!std::filesystem::exists(mp) || std::filesystem::last_write_time(mp) < std::filesystem::last_write_time(sp))
                {
                    return false;
                }
                std::filesystem::path o = outputFilePath;
                if (std::filesystem::last_write_time(o) < std::filesystem::last_write_time(sp))
                {
                    return false;
                }
            }
    ...
    for (const auto& sourceFileName : sourceFilePaths)
    {
        ...
        std::filesystem::path sfp = sourceFilePath;
        std::filesystem::path o = outputFilePath;
        if (!std::filesystem::exists(sfp) ||
            std::filesystem::last_write_time(o) < std::filesystem::last_write_time(sfp))
        {
            return false;
        }
    }
