...
{
    ProcessByte(static_cast<std::uint8_t>(0x80u));
    if (byteIndex > 56u)
    {
        while (byteIndex != 0u)
        {
            ProcessByte(static_cast<std::uint8_t>(0u));
        }
        while (byteIndex < 56u)
        {
            ProcessByte(static_cast<std::uint8_t>(0u));
    ...
    ProcessByte(static_cast<std::uint8_t>((bitCount >> 56u) & 0xFFu));
    ProcessByte(static_cast<std::uint8_t>((bitCount >> 48u) & 0xFFu));
    ProcessByte(static_cast<std::uint8_t>((bitCount >> 40u) & 0xFFu));
    ProcessByte(static_cast<std::uint8_t>((bitCount >> 32u) & 0xFFu));
    ProcessByte(static_cast<std::uint8_t>((bitCount >> 24u) & 0xFFu));
    ProcessByte(static_cast<std::uint8_t>((bitCount >> 16u) & 0xFFu));
    ProcessByte(static_cast<std::uint8_t>((bitCount >> 8u) & 0xFFu));
    ProcessByte(static_cast<std::uint8_t>(bitCount & 0xFFu));
...
void Sha1::ProcessBlock()
{
    ...
    for (int i = 0; i < 16; ++i)
    {
        ...
        w[i] = w[i] | static_cast<std::uint32_t>(block[4 * i + 1]) << 16u;
        w[i] = w[i] | static_cast<std::uint32_t>(block[4 * i + 2]) << 8u;
        w[i] = w[i] | static_cast<std::uint32_t>(block[4 * i + 3]);
    }
    for (int i = 16; i < 80; ++i)
    {
    ...
    for (int i = 0; i < 80; ++i)
    {
        ...
        std::uint32_t temp = LeftRotate(a, 5u) + f + e + k + w[i];
        e = d;
        d = c;
        c = LeftRotate(b, 30u);
        b = a;
