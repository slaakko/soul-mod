...
bool Value::IsTrue() const
{
    ...
    {
        BoolValue* boolValue = static_cast<const BoolValue*>(this);
...
Value* UIntValue::ModPowerOfTwo(Context* context) const
{
    if (value > 0u)
    {
        ...
        if (IsPowerOfTwo(static_cast<std::uint64_t>(value), shift))
        {
            return context->GetData().MakeValue(static_cast<std::uint32_t>(value - 1u), context->GetTypes());
        }
    }
    return nullptr;
}
...
std::expected<GlobalVariable*, int> Data::GetGlobalVariableForString(Value* stringValue, Type* charType)
{
    ...
    std::expected<Type*, int> rv = context->MakePtrType(charType);
    if (!rv) return std::unexpected<int>(rv.error());
    Type* ptrType = *rv;
    GlobalVariable* globalVariable = context->AddGlobalVariable(soul::ast::Span(), ptrType, context->GetNextStringValueId(), stringValue);
    globalStringVariableMap[strValue] = globalVariable;
    return std::expected<GlobalVariable*, int>(globalVariable);
}
...
std::expected<Value*, int> Data::MakeIntegerLiteral(const soul::ast::Span& span, Type* type, const std::string& strValue, const Types& types)
{
    switch (type->Id())
    {
        case boolTypeId:
        {
            if (strValue == "true")
            {
                return std::expected<Value*, int>(GetTrueValue(types));
            }
            else if (strValue == "false")
            {
                return std::expected<Value*, int>(GetFalseValue(types));
            }
            else
            {
                return Error("error making literal: Boolean value expected", span, context);
            }
            break;
        }
        case sbyteTypeId:
        {
            ...
            return std::expected<Value*, int>(GetSByteValue(static_cast<std::int8_t>(value), types));
        }
        case byteTypeId:
        {
            std::int64_t value = std::stoll(strValue);
            ...
            return std::expected<Value*, int>(GetByteValue(static_cast<std::uint8_t>(value), types));
        }
        case shortTypeId:
        {
            std::int64_t value = std::stoll(strValue);
            ...
            return std::expected<Value*, int>(GetShortValue(static_cast<std::int16_t>(value), types));
        }
        case ushortTypeId:
        {
            std::int64_t value = std::stoll(strValue);
            ...
            return std::expected<Value*, int>(GetUShortValue(static_cast<std::uint16_t>(value), types));
        }
        case intTypeId:
        {
            std::int64_t value = std::stoll(strValue);
            ...
            return std::expected<Value*, int>(GetIntValue(static_cast<std::int32_t>(value), types));
        }
        case uintTypeId:
        {
            std::int64_t value = std::stoll(strValue);
        ...
        {
            std::int64_t value = std::stoll(strValue);
            return std::expected<Value*, int>(GetLongValue(value, types));
        }
        case ulongTypeId:
        {
            std::uint64_t value = std::stoull(strValue);
            return std::expected<Value*, int>(GetULongValue(static_cast<std::int64_t>(value), types));
        }
        case floatTypeId:
        {
            float value = std::stof(strValue);
            return std::expected<Value*, int>(GetFloatValue(value, types));
        }
        case doubleTypeId:
        {
            double value = std::stod(strValue);
            return std::expected<Value*, int>(GetDoubleValue(value, types));
        }
        default:
        {
            return Error("error making literal: invalid numeric value", span, context);
        }
    }
    std::expected<Value*, int>(nullptr);
}
