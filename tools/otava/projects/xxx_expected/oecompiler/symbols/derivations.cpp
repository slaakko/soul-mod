...
{
    return writer.GetBinaryStreamWriter().Write(static_cast<std::uint8_t>(derivations));
}

std::expected<bool, int> Read(Reader& reader, Derivations& derivations)
{
    std::expected<std::uint8_t, int> rv = reader.GetBinaryStreamReader().ReadByte();
    if (!rv) return std::unexpected<int>(rv.error());
    uint8_t d = *rv;
    derivations = static_cast<Derivations>(d);
