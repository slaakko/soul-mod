...
NfaEdge::NfaEdge(Symbol* symbol_, NfaState* nxt_) noexcept : symbol(symbol_), nxt(nxt_)
{
}
...
{
    symbol->Print(formatter);
    formatter.WriteLine("-> " + std::to_string(nxt->Id()));
}
...
void NfaState::AddPrev(NfaState* prv_)
{
    prv.insert(prv_);
}
...
{
    std::vector<NfaState*> nxt;
    for (const auto& edge : edges)
    {
        Symbol* symbol = edge.GetSymbol();
        if (symbol->Match(c))
        {
            nxt.push_back(edge.Next());
        }
    }
    return nxt;
}
...
DfaState::DfaState(int id_, const std::vector<int>& nfaStateIds_) : id(id_), nfaStateIds(nfaStateIds_), nxt(), marked(false), accept(false), ruleIndex(-1)
{
}
...
void DfaState::Print(LexerContext& context, CodeFormatter& formatter)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        if (ns)
        {
            ...
            formatter.WriteLine("-> " + std::to_string(nxt[i]->Id()));
        }
    }
    formatter.DecIndent();
}
...
std::vector<int> EpsilonClosure(LexerContext& lexerContext, const std::vector<int>& stateIds)
{
    ...
    while (!s.empty())
    {
        int b = s.back();
        s.pop_back();
        NfaState* state = lexerContext.GetNfaState(b);
...
std::vector<int> Move(LexerContext& lexerContext, const std::vector<int>& stateIds, char32_t c)
{
    ...
    for (int stateId : stateIds)
    {
        ...
        {
            int s = sn->Id();
            if (std::find(nxt.cbegin(), nxt.cend(), s) == nxt.cend())
            {
                nxt.push_back(s);
            }
        }
...
Dfa Compile(LexerContext& lexerContext, Nfa& nfa)
{
    ...
    s->Mark();
    std::vector<DfaState*> st;
    st.push_back(s);
    while (!st.empty())
    {
        DfaState* state = st.back();
        st.pop_back();
        dfa.AddState(state);
        for (Class* cls : lexerContext.Partition())
        {
            if (!cls->Chars().empty())
            {
                ...
                std::vector<int> nxt = EpsilonClosure(lexerContext, Move(lexerContext, state->NfaStateIds(), c));
                if (nxt.empty())
                {
                ...
                {
                    DfaState* n = lexerContext.MakeDfaState(nxt);
                    state->AddNext(n);
                    if (!n->IsMarked())
                    {
                        n->Mark();
