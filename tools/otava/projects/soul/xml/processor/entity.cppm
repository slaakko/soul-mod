...
public:
    ...
    virtual ~Entity();
    inline EntityKind Kind() const noexcept { return kind; }
    inline const std::string& Name() const noexcept { return name; }
    virtual const std::u32string& Value() const noexcept = 0;
    inline bool IsInternal() const noexcept { return kind == EntityKind::internalEntity; }
    inline bool IsExternal() const noexcept { return kind == EntityKind::externalEntity; }
private:
    EntityKind kind;
    std::string name;
};
...
public:
    ...
    virtual const std::u32string& Value() const noexcept override { return value; }
private:
    std::u32string value;
};
...
public:
    ...
    Entity* GetEntity(const std::string& entityName) const noexcept;
private:
