...
ParentNode::ParentNode(NodeKind kind_, const soul::ast::SourcePos& sourcePos_, const std::string& name_) : 
    Node(kind_, sourcePos_, name_), firstChild(nullptr), lastChild(nullptr)
{
}
...
std::unique_ptr<Node> ParentNode::RemoveChild(Node* child)
{
    ...
    if (child->Parent() != this)
    {
        ...
        {
            errorMessage.append(1, '\n').append(soul::lexer::MakeMessage("info", "see reference", childSourcePos, *fileMap));
        }
