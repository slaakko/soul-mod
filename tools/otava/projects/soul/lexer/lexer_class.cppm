...
public:
    ...
    void ThrowExpectationFailure(std::int64_t p, const std::string& name)
    {
    ...
    LexerState<Char, LexerBase<Char>> GetState() noexcept
    {
        LexerState<Char, LexerBase<Char>> lexerState(this);
        lexerState.token = token;
        lexerState.line = line;
        lexerState.lexeme = lexeme;
        lexerState.pos = pos;
        lexerState.tokens = tokens;
        lexerState.flags = flags;
        lexerState.recordedPosPair = recordedPosPair;
        lexerState.farthestPos = farthestPos;
        lexerState.ruleContext = ruleContext;
        lexerState.farthestRuleContext = farthestRuleContext;
        lexerState.currentPos = GetPos();
        return lexerState;
    }
    void SetState(const LexerState<Char, LexerBase<Char>>& lexerState) noexcept
    {
        token = lexerState.token;
        line = lexerState.line;
        lexeme = lexerState.lexeme;
        pos = lexerState.pos;
        tokens = lexerState.tokens;
        flags = lexerState.flags;
        recordedPosPair = lexerState.recordedPosPair;
        farthestPos = lexerState.farthestPos;
        ruleContext = lexerState.ruleContext;
    ...
    TokenLineType TokenizeLine(const std::basic_string<Char>& line, int lineNumber, int startState)
    {
        ...
        if (state != 0 && state != -1)
        {
            if (blockCommentStates.find(state) != blockCommentStates.end())
            {
