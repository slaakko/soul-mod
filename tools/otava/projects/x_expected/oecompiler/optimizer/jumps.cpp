...
std::expected<bool, int> OptimizeJumps(otava::intermediate::Function* fn)
{
        ...
        if (terminator->IsJumpInstruction())
        {
            ...
            else if (terminator->IsBranchInstruction())
            {
                ...
                if (branch->Cond()->IsTrue())
                {
                    ...
                    std::expected<bool, int> rv = otava::intermediate::ReplaceInstructionWithInstruction(branch, jmp);
                    if (!rv) return rv;
                    optimized = true;
                }
