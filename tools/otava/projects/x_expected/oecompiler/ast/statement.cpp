...
HandlerNode::HandlerNode(const soul::ast::SourcePos& sourcePos_, Node* exception_, Node* catchBlock_, const soul::ast::SourcePos& lpPos_, 
    const soul::ast::SourcePos& rpPos_) : CompoundNode(NodeKind::handlerNode, sourcePos_), ex(exception_), catchBlock(catchBlock_), lpPos(lpPos_), rpPos(rpPos_)
{
}

std::expected<Node*, int> HandlerNode::Clone() const
{
    std::expected<Node*, int> e = ex->Clone();
    if (!e) return e;
