...
std::expected<std::int16_t, int> MemoryReader::ReadShort()
{
    ...
    if (!rv) return std::unexpected<int>(rv.error());
    std::uint16_t u = *rv;
    return std::expected<std::int16_t, int>(static_cast<std::int16_t>(u));
}
...
std::expected<std::int32_t, int> MemoryReader::ReadInt()
{
    ...
    if (!rv) return std::unexpected<int>(rv.error());
    std::uint32_t u = *rv;
    return std::expected<std::int32_t, int>(static_cast<std::int32_t>(u));
}
...
std::expected<std::int64_t, int> MemoryReader::ReadLong()
{
    ...
    if (!rv) return std::unexpected<int>(rv.error());
    std::uint64_t u = *rv;
    return std::expected<std::int64_t, int>(static_cast<std::int64_t>(u));
}
