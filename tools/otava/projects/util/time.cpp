...
Date GetCurrentDate() noexcept
{
    ...
    return Date(1900 + localTime->tm_year, static_cast<Month>(static_cast<std::int8_t>(1 + localTime->tm_mon)), static_cast<std::int8_t>(localTime->tm_mday));
}
...
std::string FormatTimeMs(std::int32_t milliseconds)
{
    ...
    std::string timeStr;
    timeStr.append(1, static_cast<char>(static_cast<std::int32_t>('0') + ((hh / 10) % 10)));
    timeStr.append(1, static_cast<char>(static_cast<std::int32_t>('0') + (hh % 10)));
    timeStr.append(1, ':');
    timeStr.append(1, static_cast<char>(static_cast<std::int32_t>('0') + ((mm / 10) % 10)));
    timeStr.append(1, static_cast<char>(static_cast<int>('0') + (mm % 10)));
    timeStr.append(1, ':');
    timeStr.append(1, static_cast<char>(static_cast<std::int32_t>('0') + ((ss / 10) % 10)));
    timeStr.append(1, static_cast<char>(static_cast<std::int32_t>('0') + (ss % 10)));
    timeStr.append(1, '.');
    timeStr.append(1, static_cast<char>(static_cast<std::int32_t>('0') + ((ms / 100) % 10)));
    timeStr.append(1, static_cast<char>(static_cast<std::int32_t>('0') + ((ms / 10) % 10)));
    timeStr.append(1, static_cast<char>(static_cast<std::int32_t>('0') + (ms % 10)));
    return timeStr;
}
...
DateTime GetCurrentDateTime()
{
    ...
    std::tm* localTime = nullptr;
    localTime = std::localtime(&currentTime);
    return DateTime(Date(1900 + localTime->tm_year, static_cast<Month>(static_cast<std::int8_t>(1 + localTime->tm_mon)), static_cast<std::int8_t>(localTime->tm_mday)), localTime->tm_hour * 3600 + localTime->tm_min * 60 + localTime->tm_sec);
}
...
std::string DurationStr(std::int64_t nanosecs)
{
    std::string s;
    std::int64_t hh = Hours(nanosecs) % 24;
    ...
    s.append(1, ':');
    std::int64_t mm = Minutes(nanosecs) % 60;
    ...
    s.append(1, ':');
    std::int64_t ss = Seconds(nanosecs) % 60;
    ...
    s.append(1, '.');
    std::int64_t ms = Milliseconds(nanosecs) % 1000;
    ...
    s.append(1, '.');
    std::int64_t us = Microseconds(nanosecs) % 1000;
...
std::time_t MkTime(const DateTime& dt)
{
    ...
    std::tm mtm;
    mtm.tm_year = dt.GetDate().Year() - 1900;
    mtm.tm_mon = static_cast<std::int8_t>(dt.GetDate().GetMonth()) - static_cast<std::int8_t>(Month::january);
    mtm.tm_mday = dt.GetDate().Day();
    mtm.tm_hour = dt.Hours() % 24;
    mtm.tm_min = dt.Minutes() % 60;
    mtm.tm_sec = dt.Seconds() % 60;
    mtm.tm_isdst = ltm->tm_isdst;
    return std::mktime(&mtm);
}
...
DateTime ToDateTime(time_t time)
{
    ...
    return DateTime(Date(1900 + localTime->tm_year, static_cast<Month>(static_cast<std::int8_t>(1 + localTime->tm_mon)),
        static_cast<std::int8_t>(localTime->tm_mday)), localTime->tm_hour * 3600 + localTime->tm_min * 60 + localTime->tm_sec);
}
