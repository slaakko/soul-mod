...
std::string PlatformStringToUtf8(const std::string& platformString)
{
    ...
    std::unique_ptr<char16_t> buf(static_cast<char16_t*>(operator new(bufSize)));
    int result = ort_platform_string_to_utf16_string(platformString.c_str(), buf.get(), bufSize);
...
std::string Utf8StringToPlatformString(const std::string& utf8String)
{
    ...
    const int bufSize = 4096;
    std::unique_ptr<char> buf(static_cast<char*>(operator new(bufSize)));
