...
{
public:
    BinaryStreamWriter(Stream& stream_) noexcept;
    inline Stream& GetStream() noexcept { return stream; }
    void Write(bool x);
    ...
    void WriteTime(std::time_t x);
    inline std::int64_t Position() const noexcept { return stream.Position(); }
private:
    Stream& stream;
};
