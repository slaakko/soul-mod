...
public:
    ...
    rb_tree(const rb_tree& that) : header(), count(0), key_of(), comp()
    {
        init();
        copy_from(that);
    }
    rb_tree(rb_tree&& that) : header(std::move(that.header)), count(that.count), key_of(that.key_of), comp(that.comp)
    {
        that.count = 0;
    }
