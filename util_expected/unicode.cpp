...
import util_expected.path;
import util_expected.memory.stream;
...
std::expected<std::string, int> SoulUcdFilePath()
{
    ...
    if (!rv) return rv;
    std::filesystem::path p(*rv);
    p /= std::filesystem::path("unicode");
    p /= std::filesystem::path("soul_ucd.bin");
    return std::expected<std::string, int>(p.generic_string());
}
...
std::expected<bool, int> Alias::Read(BinaryStreamReader& reader)
{
    ...
    if (!rv) return std::unexpected<int>(rv.error());
    typeId = static_cast<AliasTypeId>(*rv);
...
std::expected<bool, int> ExtendedCharacterInfo::Read(BinaryStreamReader& reader)
{
    ...
    rvb = reader.ReadByte();
    if (!rvb) return std::unexpected<int>(rvb.error());
    bidiClass = static_cast<BidiClassId>(*rvb);
    rvb = reader.ReadByte();
    if (!rvb) return std::unexpected<int>(rvb.error());
    numericType = static_cast<NumericTypeId>(*rvb);
    rvs = reader.ReadUtf8String();
    if (!rvs) return std::unexpected<int>(rvs.error());
    numericValue = *rvs;
    rvb = reader.ReadByte();
    ...
    rvb = reader.ReadByte();
    if (!rvb) return std::unexpected<int>(rvb.error());
    bidiPairedBracketType = static_cast<BidiPairedBracketTypeId>(*rvb);
...
{
    std::uint32_t n = static_cast<uint32_t>(extendedPageStarts.size());
...
{
    std::expected<uint32_t, int> rv = reader.ReadUInt();
    if (!rv) return std::unexpected<int>(rv.error());
    std::uint32_t n = *rv;
    for (std::uint32_t i = 0; i < n; ++i)
    {
...
std::expected<std::int64_t, int> CharacterTable::GetUncompressedFileSize() const
{
    ...
    return std::expected<std::int64_t, int>(std::filesystem::file_size(ucdFilePath));
}
...
std::expected<const CharacterInfo*, int> CharacterTable::GetCharacterInfo(char32_t codePoint)
{
    ...
    std::expected<CharacterInfo*, int> rv = page->GetCharacterInfo(infoIndex);
    if (!rv) return std::unexpected<int>(rv.error());
    return rv;
}
...
std::expected<ExtendedCharacterInfo*, int> CharacterTable::GetExtendedCharacterInfo(char32_t codePoint)
{
    if (codePoint > 0x10FFFF)
    {
        std::string msg("invalid Unicode code point ");
