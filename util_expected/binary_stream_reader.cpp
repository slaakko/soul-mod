...
module util_expected.binary.stream.reader;
...
std::expected<std::uint32_t, int> BinaryStreamReader::ReadUInt()
{
    ...
    if (!rv) return std::unexpected<int>(rv.error());
    std::uint8_t b3 = *rv;
    return std::expected<std::uint32_t, int>((static_cast<std::uint32_t>(b0) << 24) | (static_cast<std::uint32_t>(b1) << 16) | (static_cast<std::uint32_t>(b2) << 8) | 
