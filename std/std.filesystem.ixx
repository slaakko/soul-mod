...
bool create_directory(const path& p, const path& attributes, error_code& ec);
...
uint64_t file_size(const path& p, error_code& ec);
uint64_t hard_link(const path& p);
uint64_t hard_link(const path& p, error_code& ec);
...
void last_write_time(const path& p, file_time_type newTime, error_code& ec);
