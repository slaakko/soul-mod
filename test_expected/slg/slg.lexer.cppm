// this file has been automatically generated from 'D:/work/soul-mod/test_expected/slg/slg.lexer' using soul lexer generator eslg version 5.0.0
...
template<typename CharT>
std::expected<soul_expected::lexer::Lexer<SlgLexer<CharT>, CharT>, int> MakeLexer(const CharT* start, const CharT* end, const std::string& fileName);
...
std::expected<soul_expected::lexer::ClassMap<CharT>*, int> GetClassMap()
{
    ...
    if (!classmap)
    {
        std::expected<ClassMap<CharT>*, int> rv = soul_expected::lexer::MakeClassMap<CharT>("soul.lex.slg.classmap");
...
std::expected<soul_expected::lexer::ClassMap<CharT>*, int> GetClassMap(const std::string & moduleFileName, util::ResourceFlags resourceFlags)
{
    ...
    if (!classmap)
    {
        std::expected<ClassMap<CharT>*, int> rv = soul_expected::lexer::MakeClassMap<CharT>(moduleFileName, "soul.lex.slg.classmap", resourceFlags);
        if (!rv) return std::unexpected<int>(rv.error());
        classmap = *rv;
    }
    return classmap;
...
std::expected<soul_expected::lexer::Lexer<SlgLexer<CharT>, CharT>, int> MakeLexer(const CharT* start, const CharT* end, const std::string& fileName)
{
    ...
    auto lexer = soul_expected::lexer::Lexer<SlgLexer<CharT>, CharT>(start, end, fileName);
