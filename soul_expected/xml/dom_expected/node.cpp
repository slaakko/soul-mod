...
Node::Node(NodeKind kind_, const soul_expected::ast::SourcePos& sourcePos_, const std::string& name_) :
...
std::expected<bool, int> Node::SetPrefix(const std::string& prefix)
{
    if (IsElementNode() || IsAttributeNode())
    {
        auto colonPos = name.find(':');
    ...
    {
        return std::unexpected<int>(util::AllocateError("cannot set prefix for this node type: line=" + std::to_string(GetSourcePos().line)));
    }
