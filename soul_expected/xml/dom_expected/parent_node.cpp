...
module soul_expected.xml.parent.node;

import util_expected;
import soul_expected.xml.document;
import soul_expected.xml.document.fragment;
...
ParentNode::ParentNode(NodeKind kind_, const soul_expected::ast::SourcePos& sourcePos_, const std::string& name_) : Node(kind_, sourcePos_, name_), firstChild(nullptr), lastChild(nullptr)
{
}
...
std::expected<std::unique_ptr<Node>, int> ParentNode::RemoveChild(Node* child)
{
    ...
    if (child->Parent() != this)
    {
        ...
        {
            auto em = soul_expected::lexer::MakeMessage("error", errorMessage, sourcePos, *fileMap);
            if (!em) return std::unexpected<int>(em.error());
            errorMessage = *em;
        }
        soul_expected::ast::SourcePos childSourcePos = child->GetSourcePos();
        if (childSourcePos.IsValid() && fileMap)
        {
            auto m = soul_expected::lexer::MakeMessage("info", "see reference", childSourcePos, *fileMap);
...
std::expected<std::unique_ptr<Node>, int> ParentNode::ReplaceChild(Node* newChild, Node* oldChild)
{
    ...
    if (oldChild->Parent() != this)
    {
        ...
        soul_expected::ast::SourcePos childSourcePos = oldChild->GetSourcePos();
        if (childSourcePos.IsValid() && fileMap)
        {
            auto m = soul_expected::lexer::MakeMessage("info", "see reference", childSourcePos, *fileMap);
            if (!m) return std::unexpected<int>(m.error());
