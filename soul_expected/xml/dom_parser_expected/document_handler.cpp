...
module soul_expected.xml.document.handler;
...
DocumentHandler::DocumentHandler(soul_expected::xml::ParsingFlags flags_) : currentParentNode(nullptr), flags(flags_)
{
}

std::expected<bool, int> DocumentHandler::StartDocument(const soul_expected::ast::SourcePos& sourcePos)
{
    document.reset(new soul_expected::xml::Document(sourcePos));
...
std::expected<bool, int> DocumentHandler::Comment(const soul_expected::ast::SourcePos& sourcePos, const std::string& comment)
{
    auto rv = AddTextContent();
    if (!rv) return rv;
    return currentParentNode->AppendChild(new soul_expected::xml::Comment(sourcePos, comment));
}

std::expected<bool, int> DocumentHandler::PI(const soul_expected::ast::SourcePos& sourcePos, const std::string& target, const std::string& data)
{
    auto rv = AddTextContent();
    if (!rv) return rv;
    rv = currentParentNode->AppendChild(new soul_expected::xml::ProcessingInstruction(sourcePos, target, data));
    if (!rv) return rv;
    return std::expected<bool, int>(true);
}

std::expected<bool, int> DocumentHandler::Text(const soul_expected::ast::SourcePos& sourcePos, const std::string& text)
{
...
std::expected<bool, int> DocumentHandler::CDataSection(const soul_expected::ast::SourcePos& sourcePos, const std::string& cdata)
{
    auto rv = AddTextContent();
    if (!rv) return rv;
    rv = currentParentNode->AppendChild(new soul_expected::xml::CDataSection(sourcePos, cdata));
    if (!rv) return rv;
    return std::expected<bool, int>(true);
}

std::expected<bool, int> DocumentHandler::StartElement(const soul_expected::ast::SourcePos& sourcePos, const std::string& namespaceUri, const std::string& localName, const std::string& qualifiedName,
    const soul_expected::xml::processor::Attributes& attributes)
{
    auto rv = AddTextContent(true);
    if (!rv) return rv;
    parentNodeStack.push(currentParentNode);
    elementStack.push(std::move(currentElement));
    currentElement.reset(new soul_expected::xml::Element(sourcePos, qualifiedName));
...
std::expected<bool, int> DocumentHandler::SkippedEntity(const soul_expected::ast::SourcePos& sourcePos, const std::string& entityName)
{
...
std::expected<bool, int> DocumentHandler::AddTextContent(bool addSpace)
{
    if (currentElement)
    {
        ...
        if (!textContent.empty())
        {
            ...
            auto rv = currentElement->AppendChild(new soul_expected::xml::Text(textSourcePos, textContent));
            if (!rv) return rv;
            textSourcePos = soul_expected::ast::SourcePos();
        }
    }
    textContent.clear();
    return std::expected<bool, int>(true);
