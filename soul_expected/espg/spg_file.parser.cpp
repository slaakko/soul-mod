                        ...
                        while (true)
                        {
                            std::int64_t save = lexer.GetPos();
                            {
                                ...
                                soul_expected::parser::Match* parentMatch10 = &match;
                                {
                                    ...
                                    {
                                        std::int64_t pos = lexer.GetPos();
                                        std::expected<soul_expected::parser::Match, int> m = soul_expected::spg::spg::file::parser::SpgFileParser<LexerT>::SpgFileDeclaration(lexer);
                                        if (!m) return std::unexpected<int>(m.error());
                                        soul_expected::parser::Match match = *m;
...
std::expected<soul_expected::parser::Match, int> SpgFileParser<LexerT>::SpgFileDeclaration(LexerT& lexer)
{
    ...
    soul_expected::parser::Match* parentMatch0 = &match;
    {
        ...
        {
            std::int64_t pos = lexer.GetPos();
            std::expected<soul_expected::parser::Match, int> m = soul_expected::spg::spg::file::parser::SpgFileParser<LexerT>::ParserFileDeclaration(lexer);
            if (!m) return std::unexpected<int>(m.error());
            soul_expected::parser::Match match = *m;
        ...
        *parentMatch0 = match;
        if (!match.hit)
        {
            ...
            lexer.SetPos(save);
            {
                ...
                {
                    std::int64_t pos = lexer.GetPos();
                    std::expected<soul_expected::parser::Match, int> m = soul_expected::spg::spg::file::parser::SpgFileParser<LexerT>::TokenFileDeclaration(lexer);
                    if (!m) return std::unexpected<int>(m.error());
                    soul_expected::parser::Match match = *m;
