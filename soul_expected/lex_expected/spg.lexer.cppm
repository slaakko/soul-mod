...
export module soul_expected.lex.spg;
...
export namespace soul_expected::lex::spg {
...
template<typename CharT>
std::expected<soul_expected::lexer::Lexer<SpgLexer<CharT>, CharT>, int> MakeLexer(const CharT* start, const CharT* end, const std::string& fileName);

template<typename CharT>
std::expected<soul_expected::lexer::Lexer<SpgLexer<CharT>, CharT>, int> MakeLexer(const std::string& moduleFileName, util::ResourceFlags resourceFlags, const CharT* start, const CharT* end, const std::string& fileName);

soul_expected::ast::common::TokenCollection* GetTokens();

struct SpgLexer_Variables : public soul_expected::lexer::Variables
{
    SpgLexer_Variables();
    int leftAngleCount;
    bool matchFilePath;
...
struct SpgLexer
{
    ...
    static std::int64_t GetTokenId(std::int32_t ruleIndex, soul_expected::lexer::LexerBase<CharT>& lexer)
    {
        switch (ruleIndex)
        {
            ...
            {
                lexer.Retract();
                return soul_expected::punctuation::token::PRODUCES;
                break;
            }
            case 7:
            {
...
std::expected<soul_expected::lexer::ClassMap<CharT>*, int> GetClassMap()
{
    ...
    if (!classmap)
    {
        std::expected<soul_expected::lexer::ClassMap<CharT>*, int> rv = soul_expected::lexer::MakeClassMap<CharT>("soul_expected.lex.spg.classmap");
...
std::expected<soul_expected::lexer::ClassMap<CharT>*, int> GetClassMap(const std::string & moduleFileName, util::ResourceFlags resourceFlags)
{
    ...
    if (!classmap)
    {
        std::expected<soul_expected::lexer::ClassMap<CharT>*, int> rv = soul_expected::lexer::MakeClassMap<CharT>(moduleFileName, "soul_expected.lex.spg.classmap", resourceFlags);
        if (!rv) return std::unexpected<int>(rv.error());
        classmap = *rv;
    }
    return classmap;
...
std::expected<soul_expected::lexer::Lexer<SpgLexer<CharT>, CharT>, int> MakeLexer(const CharT* start, const CharT* end, const std::string& fileName)
{
    ...
    auto lexer = soul_expected::lexer::Lexer<SpgLexer<CharT>, CharT>(start, end, fileName);
