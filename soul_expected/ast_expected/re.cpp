...
module soul_expected.ast.re;

import soul_expected.ast.slg;
import util_expected;

namespace soul_expected::ast::re {
...
std::expected<Nfa, int> LexerContext::MakeExpr(const std::string& id)
{
    ...
    if (expressions)
    {
        soul_expected::ast::slg::Expression* expr = expressions->GetExpression(id);
        if (expr)
        {
            ...
            if (exprParser)
            {
                soul_expected::ast::slg::Expression* prevExpression = currentExpression;
