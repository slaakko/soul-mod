...
export module soul_expected.lexer.cls;
...
import soul_expected.lexer.token.parser;
import soul_expected.lexer.base;
import soul_expected.lexer.concepts;
...
struct LexerState
{
    ...
    soul_expected::lexer::Token<CharT, LexerBaseT> token;
    int line;
    Lexeme<CharT> lexeme;
    const CharT* pos;
    std::vector<soul_expected::lexer::Token<CharT, LexerBaseT>> tokens;
    LexerFlags flags;
    soul_expected::ast::lexer::pos::pair::LexerPosPair recordedPosPair;
...
template<typename Machine, typename CharT>
    requires FiniteStateMachine<Machine, CharT>
class Lexer : public LexerBase<CharT>
{
public:
    using LexerType = Lexer<Machine, CharT>;
    using CharType = CharT;
    using TokenType = soul_expected::lexer::Token<CharT, LexerBase<CharT>>;
    using TokenLineType = soul_expected::lexer::TokenLine<CharT, LexerBase<CharT>>;
    ...
    std::expected<soul_expected::ast::Span, int> GetSpan() const override
    {
        return GetSpan(GetPos());
    }
    std::expected<soul_expected::ast::Span, int> GetSpan(std::int64_t pos) const override
    {
    ...
    soul_expected::lexer::ClassMap<CharT>* GetClassMap() const override
    {
        return classMap;
    }
    void SetClassMap(soul_expected::lexer::ClassMap<CharT>* classMap_) override
    {
        classMap = classMap_;
    }
    soul_expected::ast::slg::TokenCollection* GetTokenCollection() const override
    {
        return tokenCollection;
    }
    void SetTokenCollection(soul_expected::ast::slg::TokenCollection* tokenCollection_) override
    {
    ...
    std::int64_t GetKeywordToken(const Lexeme<CharT>& lexeme) const override
    {
        ...
        {
            return soul_expected::lexer::INVALID_TOKEN;
        }
    }
    ...
    soul_expected::lexer::Token<CharT, LexerBase<CharT>>& CurrentToken() override
    {
        return token;
    }
    ...
    std::expected<soul_expected::ast::SourcePos, int> GetSourcePos(std::int64_t pos) const
    {
        const CharT* s = start;
        ...
        return soul_expected::ast::SourcePos(pos, file, line, col);
    }
    ...
    {
        std::expected<soul_expected::ast::SourcePos, int> srv = GetSourcePos(pos);
        if (!srv) return std::unexpected<int>(srv.error());
        soul_expected::ast::SourcePos sourcePos = *srv;
    ...
    soul_expected::lexer::Variables* GetVariables() const override
    {
        return const_cast<soul_expected::lexer::Variables*>(static_cast<const soul_expected::lexer::Variables*>(&vars));
    }
    const CharT* Start() const
    {
        return start;
    }
    ...
    const std::vector<soul_expected::lexer::Token<CharT, LexerBase<CharT>>>& Tokens() const
    {
        return tokens;
    }
    std::vector<soul_expected::lexer::Token<CharT, LexerBase<CharT>>> ReleaseTokens()
    {
    ...
    void BeginRecordedParse(const soul_expected::ast::lexer::pos::pair::LexerPosPair& lexerPosPair) override
    {
        PushState();
    ...
    TokenLineType TokenizeLine(const std::basic_string<CharT>& line, int lineNumber, int startState)
    {
        ...
        lexeme.end = end;
        token.match = lexeme;
        token.id = soul_expected::lexer::INVALID_TOKEN;
...
private:
    std::expected<bool, int> NextToken()
    {
        ...
        while (true)
        {
            ...
            if (state == -1)
            {
                if (token.id == soul_expected::lexer::CONTINUE_TOKEN)
                {
                    if (pos == end)
                    {
                        break;
                ...
                else if (token.id == soul_expected::lexer::INVALID_TOKEN)
                {
                    if (pos == end)
                    {
                        break;
                    ...
                    else
                    {
                        ...
                        return std::unexpected<int>(util::AllocateError(
                            "soul_expected::lexer::Lexer::NextToken(): error: invalid character '" + *u + "' in file '" + fileName + "' at line " + std::to_string(line)));
                    }
                }
                else
                {
        ...
        std::int64_t p = -1;
        if (token.id != soul_expected::lexer::INVALID_TOKEN && token.id != soul_expected::lexer::CONTINUE_TOKEN)
        {
            tokens.push_back(token);
            current = tokens.end() - 1;
            p = GetPos();
        }
        TokenType endToken(soul_expected::lexer::END_TOKEN, this);
        endToken.match.begin = end;
        endToken.match.end = end;
        tokens.push_back(endToken);
        if (p == -1)
    ...
    soul_expected::ast::lexer::pos::pair::LexerPosPair recordedPosPair;
    CharT separatorChar;
    std::vector<soul_expected::lexer::Token<CharT, LexerBase<CharT>>> tokens;
    std::vector<soul_expected::lexer::Token<CharT, LexerBase<CharT>>>::iterator current;
    soul_expected::lexer::Token<CharT, LexerBase<CharT>> token;
    Lexeme<CharT> lexeme;
    std::int32_t file;
    std::int32_t line;
    const CharT* start;
