...
// this file has been automatically generated from 'C:/work/soul-mod/soul/cpp20/parser/guard.parser' using soul parser generator spg version 4.1.0
...
template<typename Lexer>
soul::parser::Match GuardParser<Lexer>::NoDeclSpecDeclarationGuard(Lexer& lexer, soul::cpp20::symbols::Context* context)
{
    #ifdef SOUL_PARSER_DEBUG_SUPPORT
    int64_t parser_debug_match_pos = 0;
    bool parser_debug_write_to_log = lexer.Log() != nullptr;
    ...
    {
        parser_debug_match_pos = lexer.GetPos();
        soul::lexer::WriteBeginRuleToLog(lexer, "NoDeclSpecDeclarationGuard");
    }
    #endif // SOUL_PARSER_DEBUG_SUPPORT
    ...
    soul::parser::Match* parentMatch0 = &match;
    {
        ...
        if (match.hit)
        {
            pass = context->EnableDeclSpecFunctionDeclaration();
        }
        if (match.hit && !pass)
        {
            match = soul::parser::Match(false);
    ...
    if (parser_debug_write_to_log)
    {
        if (match.hit) soul::lexer::WriteSuccessToLog(lexer, parser_debug_match_pos, "NoDeclSpecDeclarationGuard");
        else soul::lexer::WriteFailureToLog(lexer, "NoDeclSpecDeclarationGuard");
    }
    #endif // SOUL_PARSER_DEBUG_SUPPORT
    if (!match.hit)
    {
