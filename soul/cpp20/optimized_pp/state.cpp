...
void State::PopSection()
{
    if (sectionStack.empty())
    {
        ...
        throw std::runtime_error("soul.cpp20.pp.state: error: section stack is empty");
    }
...
void State::SkipSection()
{
    lexer->Skip(true);
    lexer->Increment();
    while (lexer->CurrentToken().id != soul::lexer::END_TOKEN && lexer->CurrentToken().id != soul::cpp20::token::PP_LINE)
    {
        lexer->Increment();
    }
    lexer->Skip(false);
    if (lexer->CurrentToken().id == soul::cpp20::token::PP_LINE)
    {
